        ...
        success: function(respuesta) {
                        ...
                        switch (caso) {
                            case 1:
                                html += '<td class="border-dashed border-t border-gray-200 ">' +
                                    '<span class="text-gray-700 px-6 py-3 flex items-right">' + numberFormat2.format(parseFloat(respuesta[i][total]).toFixed(2)) + '</span>' +
                                    '</td>';
                                anio = anio + 1;
                                break;
                            case 2:
            ...
            if (forma == 1) {
                var html = '<table class="border-collapse table-auto w-full whitespace-no-wrap bg-white table-striped relative">' +
                    '<thead>' +
                    '<tr class="text-left">' +
                    '<th class="bg-gray-100 sticky top-0 border-b border-gray-200 px-6 py-2 text-gray-600 font-bold tracking-wider uppercase text-xs fijo" style="z-index:2;">'
                    if(agrupacion == 1){
                        html +='Comisionista' +
                        '</th>';
                    }else if(agrupacion == 2){
            ...
            if (forma == 3) {
                ...
                fechadia = 0;
                for (i = 0; i < respuesta.length; i++) {
                    fechadia = new Date(respuesta[i].fecha);
                    fechadia = dayOfYear(fechadia);
                    ...
                    if (respuesta[i][cod] == prescriptor) {
                        ...
                        //console.log(saltoDatosM);                                                       
                        var incrementacion = (saltoDatosA * 365) + saltoDatosM;
                        console.log(incrementacion);
                        for (var q = 1; q <= incrementacion - 1; q++) {
                            html += '<td class="border-dashed border-t border-gray-200">' +
                                '</td>';
                        }
                        ...
                        if (fechadia == dayOfYear(fechaFin) && respuesta[i].anio == fechaInicio.getFullYear()) {
                            html += '<td class="border-dashed border-t border-gray-200">' +
                                '<span class="text-gray-700 px-6 py-3 flex items-right">' + numberFormat2.format(parseFloat(respuesta[i][total]).toFixed(2)) + '</span>' +
                                '</td>';
                            dia = dayOfYear(respuesta[i].fecha);
                            anio = respuesta[i].anio;
                        } else {
                            var diferenciaA = respuesta[i].anio - fechaInicio.getFullYear();
