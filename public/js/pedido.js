 ...
 async function rellenarListadoArticuloPedido(articulo) {
    ...
    setTimeout(function() {
        ...
        setTimeout(function() {        
            ...
            var partida;
            var fechaCaducidad;
            html += "<tr class='linea-terminada " + codigoArticulo +"-"+ orden +"'>" +
                "<td><button type='button' class='btn mr-2 text-danger eliminarProducto' onclick='eliminarProductoTabla(this.id)' id='" + codigoArticulo + "-"+orden+"'><i class='fas fa-trash-alt\ ml-3'></i></button>" +
                codigoArticulo + "</td>";
                
            if(codigoArticulo != '010'){
                if(codigoArticulo != '011'){
                html += "<td>" + articulo.DescripcionArticulo + 
                "<button class='btn btn-secundary' style='size:20px;' name='"+codigoArticulo+"' onclick='ultimosPedidos(this.name,"+codigoCliente+")'><i class='fas fa-eye'></i></button>"+
                "</td>" +                
                "<td hidden>";
                }else{
                html += "<td class='d-flex justify-content-between'><input type='text' class='border' id='descripcion-" +  codigoArticulo + "-"+orden + "' onblur='descripcion0(this.value,this.id)'>"+                
                "<button class='btn btn-secundary' style='width:50px; border:none;' name='"+codigoArticulo+"' onclick='ultimosPedidos(this.name,"+codigoCliente+")'><i class='fas fa-eye'></i></button>"+                    
                    "<input type='number'class='border ' id='precioCoste-"+codigoArticulo+"-"+orden+"' onblur='precioCosteProducto(this.value,this.id)' style='width:50px; border:none;' placeholder='coste'>"+
                "</td>" +
                "<td hidden>";
                }
            }else{                    
                html += "<td><input type='text' class='border' id='descripcion-" +  codigoArticulo + "-"+orden + "' onblur='descripcion0(this.value,this.id)'>"+
                "<button class='btn btn-secundary' style='width:50px; border:none;' name='"+codigoArticulo+"' onclick='ultimosPedidos(this.name,"+codigoCliente+")'><i class='fas fa-eye'></i></button>"+                    
                    "<input type='number'class='border ' id='precioCoste-"+codigoArticulo+"-"+orden+"' onblur='precioCosteProducto(this.value,this.id)' style='width:50px; border:none;' placeholder='coste'>"+
                "</td>" +
                "<td hidden>";
            ...
            if (tratamientoPartida == true) {
                ...
                html += "</select>";
                html += "<input type='hidden' class='partida' id='" + codigoArticulo + "-"+orden+"partida'  value='" + partida + "'/>" +
                    "<input type='hidden' class='fechaCaducidad' id='" + codigoArticulo +"-"+orden+ "fechacaducidad' value='" + fechaCaducidad + "'/>";
            } else {
                html += "No tiene partida";
                html += "<input type='hidden' class='partida' id='" + codigoArticulo +"-"+orden+ "partida'  value='0'/>" +
                    "<input type='hidden' class='fechaCaducidad' id='" + codigoArticulo + "-"+orden+"fechacaducidad' value='0'/>";
            }
            
            html += "</td>" +
                "<td class='text-right'><input type='number' class='"+codigoArticulo+"-"+orden+"pvp_articulo' name='pvpArticulo' id='pvpArticulo-"+codigoArticulo+"-"+orden+"' value='"  + parseFloat(articulo.PrecioVenta, 2) + "' min='0' onchange='actualizarPvpProducto(this.id)' style='width:50px; border:none; text-align: right!important;'> </td>";
            ...
            if(tarifasEspeciales.tipo == 'familia'){
                 html += "<td><input type='number' class='" + codigoArticulo + "-"+orden+"dto_articulo' name='descuentoArticulo' id='descuentoArticulo-" + codigoArticulo +"-"+orden+ "' value='"+parseFloat(descuento,2)+"' min='0' max='100' onchange='actualizarPvpProducto(this.id)' disabled style='width:50px; border:none; text-align: right!important;'>%</td>";

            }else{
                 html += "<td><input type='number' class='" + codigoArticulo + "-"+orden+"dto_articulo' name='descuentoArticulo' id='descuentoArticulo-" + codigoArticulo +"-"+orden+ "' value='"+parseFloat(descuento,2)+"' min='0' max='100' onchange='actualizarPvpProducto(this.id)'style='width:50px; border:none; text-align: right!important;'>%</td>";        

            }

            html += "<td class='text-right'>" + parseFloat(importeLiquido.toFixed(2)) + "</td>" +
                "<td><input type='number' class='" + codigoArticulo + "-"+orden+"cantidadProducto' name='cantidadProducto' id='cantidadProducto-" + codigoArticulo + "-"+orden+"' value='" + cantidadProducto + "' onchange='actualizarPvpProducto(this.id)' style='width:50px; border:none;'></td>" +
                "<td class='text-right'>" + subTotal.toFixed(2) +                 
                "</td>" +
