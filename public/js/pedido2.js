 ...
 */
 async function rellenarListadoArticuloPedido(articulo) {
    ...
    $.ajax({
        data: parametros,
        url: './comprobarTratamientoPartidas',
        type: 'post',
        timeout: 2000,
        async: true,
        success: function(response) {
        ...
        setTimeout(function() {        
            ...
            if(codigoArticulo != '010'){
                ...
                html += "<td class='px-6 py-4 text-gray-900'>" +
                    "<div flex flex-cols-12>"+
                        ...
                        "<div id='divObservacionArticulo" + codigoArticulo +"-"+ orden +"' style='display: none;'>"+
                            ...
                            "<textarea type='text' id='observacionArticulo-"+codigoArticulo+"-"+ orden+"' class='focus:ring-indigo-500 focus:border-indigo-500 flex-2 rounded block sm:text-sm w-full' rows='1' onblur='observacionArticulo(this.value, this.id)'></textarea>"+
                        "</div>"+
                    "</div>"+
                "</td>" +                
                "<td hidden>";
                ...
                "<button <button type='button' class='text-green-400' style='width:50px; border:none;' name='"+codigoArticulo+"' onclick='ultimosPedidos(this.name,"+codigoCliente+")'><i class='fas fa-eye'></i></button>"+                    
                    "<input type='number'class='border text-gray-900 ' id='precioCoste-"+codigoArticulo+"-"+orden+"' onblur='precioCosteProducto(this.value,this.id)' style='width:50px; border:none;' placeholder='coste'>"+
                "</td>" +
                "<td hidden>";
                }
            }else{                    
            ...
            html += "</td>" +
                "<td  class='px-6 py-4 text-right'><input type='number' class='"+codigoArticulo+"-"+orden+"pvp_articulo text-gray-900' name='pvpArticulo' id='pvpArticulo-"+codigoArticulo+"-"+orden+"' value='"  + parseFloat(articulo.PrecioVenta, 2) + "' min='0' onchange='actualizarPvpProducto(this.id)' style='width:50px; border:none; text-align: right!important;'> </td>";
            ...
            if(tarifasEspeciales.tipo == 'familia'){
                 html += "<td class='px-6 py-4 text-right '><input type='number' class='" + codigoArticulo + "-"+orden+"dto_articulo text-gray-900' name='descuentoArticulo' id='descuentoArticulo-" + codigoArticulo +"-"+orden+ "' value='"+parseFloat(descuento,2)+"' min='0' max='100' onchange='actualizarPvpProducto(this.id)' disabled style='width:50px; border:none; text-align: right!important;'>%</td>";
            ...
            html += "<td class='px-6 py-4 text-right text-gray-900'>" + parseFloat(importeLiquido.toFixed(2)) + "</td>" +
                "<td class='px-6 py-4 text-right'><input type='number' class='" + codigoArticulo + "-"+orden+"cantidadProducto text-right text-gray-900' name='cantidadProducto' id='cantidadProducto-" + codigoArticulo + "-"+orden+"' value='" + cantidadProducto + "' onchange='actualizarPvpProducto(this.id)' style='width:50px; border:none;'></td>" +
                ...
                //"<input type='hidden' class='iva_articulo' id='"+codigoArticulo+"_iva' value='"+ivaProducto/100+"'>"+
                "<input type='hidden' class='recargo_articulo' id='"+codigoArticulo+"_"+orden+"recargo' value='"+recargoProducto+"'>"+
                "<input type='hidden' class='prontopago_articulo' id='"+codigoArticulo+"_"+orden+"prontopago' value='"+importeProntoPago+"'>"+
 ...
 */
 async function actualizarPvpProducto(codigoArticulo) {
    ...
    $("." + codigoABuscar[1]+"-"+orden + "").children("td")[5].innerText = parseFloat(valorConIva).toFixed(2);
    $("." + codigoABuscar[1]+"-"+orden + "").children("td")[7].innerText = parseFloat(valorIvaCantida).toFixed(2);
    $("#" + codigoABuscar[1]+"-"+orden + "1").val(valorIvaCantida);
    $("#" + codigoABuscar[1]+"-"+orden + "2").val(nuevoDescuento);
...
function recalcularLineasPedido(lineas){
    ...
    for(let i = 0; i<lineas.length; i++){
        ...
        $("." + lineas[i].CodigoArticulo+"-"+lineas[i].Orden + "").children("td")[7].innerText = parseFloat(baseImponible).toFixed(2);
        $("#" + lineas[i].CodigoArticulo+"-"+lineas[i].Orden + "1").val(parseFloat(baseImponible).toFixed(2));
        //$("#" + lineas[i][CodigoArticulo]+"-"+lineas[i][Orden] + "2").val(lineas[i][BaseImponible]);
        $("#" + lineas[i].CodigoArticulo+"_"+lineas[i].Orden+"recargo").val(parseFloat(recargo).toFixed(2));
