...
function main() {
    ...
    $(".cerrarSalidaProductoPedido").click(function() {
        ...
        $("#busquedaProductoPedidoDescripicion").val("");        
...
function datosClientePedido() {
    ...
    $.ajax({
        ...
        success: function(response) {            
            ...
            email = 'mailto:'+response[0].EMail1+'?subject='+response[0].RazonSocial+' Pedido:';            
...
function buscarArticulosPedido() {
    ...
    $('#busquedaProductoPedidoDescripicion').keyup(function() {
...
function obtenerDatosUltimoPedidoPendientePuntoVenta() {
    ...
    $.ajax({
        data: parametros,
        url: './obtenerUltimoPedidoPendiente',
        type: 'post',
        timeout: 2000,
        async: true,
        success: function(response) {
 ...
 async function rellenarListadoArticuloPedido(articulo) {
    ...
    $.ajax({
        ...
        error : function(xhr, status) {
            Swal.fire({
                title: 'Error!',
                text: 'error al solicitar el orden',
                icon: 'error',
    ...
    $.ajax({
        ...
        error : function(xhr, status) {
            Swal.fire({
                title: 'Error!',
                text: 'error al comprobar tarifa',
                icon: 'error',
                confirmButtonText: 'Cerrar'
            })
        },        
    ...
    $.ajax({
        ...
        success: function(response) {
            console.log(response);
            if(response[0].Recargo === 'undefined'){
                recargoProducto = 0;
                
            }else{                
                recargoProducto = response[0].Recargo;
            }
            //recargoProducto = response[0].Recargo;
            console.log(recargoProducto);
        },
        error : function(xhr, status) {
            Swal.fire({
                title: 'Error!',
                text: 'error al comprobar iva o recargo',
                icon: 'error',
                confirmButtonText: 'Cerrar'
            })
        },
    ...
    setTimeout(function() {
        ...
        var cantidadProducto = $("#cantidadProducto\\|"+articulo.CodigoArticulo.replace(/[/]/g, "\\/")+"").val();
        //console.log($("#cantidadProducto\\|"+articulo.CodigoArticulo));
        if (cantidadProducto == undefined || cantidadProducto == 0){
            cantidadProducto = 1;
        }
        ...
        if(consultaTarifa == true){
            ...
            // }
            articulo.PrecioVenta = tarifasEspeciales.precioVenta;
            descuento = tarifasEspeciales.Descuento1;
            descuento2 = tarifasEspeciales.Descuento2;            
        }
        
        if (articulo.GrupoIva) {
        ...
        var prontoPago = $('#descuentoProntoPago').val();
        
        var importeNeto = (articulo.PrecioVenta - (descuento * articulo.PrecioVenta / 100));
        //var importeNeto = articulo.PrecioVenta - (0 * articulo.PrecioVenta / 100 );
        
        // var importeLiquido = importeNeto + (importeNeto * ivaProducto);
        var importeLiquido = importeNeto;
        var pp = parseFloat(prontoPago)/100;
        ...
        recargoProducto = subTotal * (recargoProducto/100);
        var importeProntoPago = subTotal * pp;        
        ...
        setTimeout(function() {        
            ...
            var partida;
            var fechaCaducidad;
            html += "<tr class='linea-terminada " + codigoArticulo +"|"+ orden +"  border-b dark:bg-gray-800 dark:border-gray-700 odd:bg-white even:bg-gray-50 odd:dark:bg-gray-800 even:dark:bg-gray-700'>" +
                "<td scope='row' class='px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap'><button type='button' class='mr-2 text-red-400 eliminarProducto' onclick='eliminarProductoTabla(this.id)' id='" + codigoArticulo + "|"+orden+"'><i class='fas fa-trash-alt\ ml-3'></i></button>" +
                codigoArticulo + "</td>";
                
            if(codigoArticulo != '010'){
                if(codigoArticulo != '011'){
                html += "<td class='px-6 py-4 text-gray-900'>" +
                    "<div flex flex-cols-12>"+
                        "<div>"+
                            "<a id='NodivObservacionArticulo"+codigoArticulo +"|"+ orden+"' onclick='showObservacionArticulo(this.id)'>"+articulo.DescripcionArticulo +"</a>"+
                            " <button type='button' class='text-green-400' style='size:20px;' name='"+codigoArticulo+"' onclick='ultimosPedidos(this.name,"+codigoCliente+")'><i class='fas fa-eye'></i></button>"+
                        "</div>"+
                        "<div id='divObservacionArticulo" + codigoArticulo +"|"+ orden +"' style='display: none;'>"+
                            "<label for='observacion articulo' class='block text-sm font-medium text-gray-700'> 0bservaciones Artículo </label>"+                                                                                                            
                            "<textarea type='text' id='observacionArticulo|"+codigoArticulo+"|"+ orden+"' class='observacionArticulo focus:ring-indigo-500 focus:border-indigo-500 flex-2 rounded block sm:text-sm w-full' rows='1' onblur='observacionArticulo(this.value, this.id)'></textarea>"+
                        "</div>"+
                    "</div>"+
                "</td>" +                
                "<td hidden>";
                }else{
                html += "<td class='px-6 py-4 flex justify-around'><input type='text' class='border text-gray-900' id='descripcion|" +  codigoArticulo + "|"+orden + "' onblur='descripcion0(this.value,this.id)'>"+                
                "<button <button type='button' class='text-green-400' style='width:50px; border:none;' name='"+codigoArticulo+"' onclick='ultimosPedidos(this.name,"+codigoCliente+")'><i class='fas fa-eye'></i></button>"+                    
                    "<input type='number'class='border text-gray-900 precioCoste' id='precioCoste|"+codigoArticulo+"|"+orden+"' onblur='precioCosteProducto(this.value,this.id)' style='width:50px; border:none;' placeholder='coste'>"+
                "</td>" +
                "<td hidden>";
                }
            }else{                    
                html += "<td class='px-6 py-4'><input type='text' class='border text-gray-900 ' id='descripcion|" +  codigoArticulo + "|"+orden + "' onblur='descripcion0(this.value,this.id)'>"+
                "<button <button type='button' class='text-green-400' style='width:50px; border:none;' name='"+codigoArticulo+"' onclick='ultimosPedidos(this.name,"+codigoCliente+")'><i class='fas fa-eye'></i></button>"+                    
                    "<input type='number'class='border text-gray-900 precioCoste' id='precioCoste|"+codigoArticulo+"|"+orden+"' onblur='precioCosteProducto(this.value,this.id)' style='width:50px; border:none;' placeholder='coste'>"+
                "</td>" +
            ...
            if (tratamientoPartida == true) {
                html += "<select  id='partida' name='partida' class='text-gray-900'> ";
                ...
                html += "</select>";
                html += "<input type='hidden' class='partida text-gray-900' id='" + codigoArticulo + "|"+orden+"partida'  value='" + partida + "'/>" +
                    "<input type='hidden' class='fechaCaducidad' id='" + codigoArticulo +"|"+orden+ "fechacaducidad' value='" + fechaCaducidad + "'/>";
            } else {
                html += "No tiene partida";
                html += "<input type='hidden' class='partida text-gray-900' id='" + codigoArticulo +"|"+orden+ "partida'  value='0'/>" +
                    "<input type='hidden' class='fechaCaducidad' id='" + codigoArticulo + "|"+orden+"fechacaducidad' value='0'/>";
            }
            
            html += "</td>" +
                "<td  class='px-6 py-4 text-right'><input type='number' class='"+codigoArticulo+"|"+orden+"pvp_articulo text-gray-900' name='pvpArticulo' id='pvpArticulo|"+codigoArticulo+"|"+orden+"' value='"  + parseFloat(articulo.PrecioVenta, 2) + "' min='0' onchange='controlarVacios(this.id),actualizarPvpProducto(this.id)' style='width:50px; border:none; text-align: right!important;'> </td>";
            ...
            if(tarifasEspeciales.tipo == 'familia'){
                 html += "<td class='px-6 py-4 text-right '><input type='number' class='" + codigoArticulo + "|"+orden+"dto_articulo text-gray-900' name='descuentoArticulo' id='descuentoArticulo|" + codigoArticulo +"|"+orden+ "' value='"+parseFloat(descuento,2)+"' min='0' max='100' onchange='controlarVacios(this.id),actualizarPvpProducto(this.id)' disabled style='width:50px; border:none; text-align: right!important;'>%</td>";
            ...
            html += "<td class='px-6 py-4 text-right text-gray-900'>" + parseFloat(importeLiquido.toFixed(2)) + "</td>" +
                "<td class='px-6 py-4 text-right'><input type='number' class='" + codigoArticulo + "|"+orden+"cantidadProducto text-right text-gray-900' name='cantidadProducto' id='cantidadProducto|" + codigoArticulo + "|"+orden+"' value='" + cantidadProducto + "' onchange='controlarVacios(this.id),actualizarPvpProducto(this.id)' style='width:50px; border:none;'></td>" +
                "<td class='px-6 py-4 text-right text-gray-900'>" + subTotal.toFixed(2) +                 
                "</td>" +
                "<input type='hidden' id='" + codigoArticulo+"|"+orden+ "1' class='subTotal' value='" + subTotal.toFixed(2) + "'/>" +
                "<input type='hidden' class='codigoArticulo' value='" + codigoArticulo + "' />" +
                "<input type='hidden' class='precio' value='" + importeLiquido + "' />" +
                "<input type='hidden' id='" + codigoArticulo +"|"+orden+ "cantidad' class='cantidad' value='" + cantidadProducto + "' />" +
                ...
                "<input type='hidden' class='descuento_cliente' id='descuento_cliente_pedido' value='0' />" +
                "<input type='hidden' class='descuento_articulo' id='" + codigoArticulo+ "2' value='" + parseFloat(descuento, 2) + "' />" +
                "<input type='hidden' class='iva_articulo' id='"+codigoArticulo+"|"+orden+"_iva' value='"+ivaProducto+"'>"+
                "<input type='hidden' class='precioCompra_articulo' id='"+codigoArticulo+"|"+orden+"_precioCompra' value='"+articulo.PrecioCompra+"'>"+
                "<input type='hidden' class='recargo_articulo' id='"+codigoArticulo+"|"+orden+"recargo' value='"+recargoProducto+"'>"+
                "<input type='hidden' class='prontopago_articulo' id='"+codigoArticulo+"|"+orden+"prontopago' value='"+importeProntoPago+"'>"+
                "<input type='hidden' class='"+codigoArticulo+"|"+orden+"linea_articulo' id='" + codigoArticulo+"|"+orden+ "3' value='"+orden+"' />" +
                "<input type='hidden' class='ordenLinea' value='"+orden+"' />" +
                "</tr>";
...
function controlarVacios(e){
    ...
    let precio = $('#pvpArticulo\\|' + id[1] + '\\|'+ id[2]).val();
    if(precio == 0 || precio == ''){
        $('#pvpArticulo\\|' + id[1] + '\\|'+ id[2]).val(0);
    }
    let descuento = $('#descuentoArticulo\\|' + id[1] + '\\|'+ id[2]).val();
    if(descuento == 0 || descuento == ''){
        $('#descuentoArticulo\\|' + id[1] + '\\|'+ id[2]).val(0);
    }
    let unidades = $('#cantidadProducto\\|' + id[1] + '\\|'+ id[2]).val();
    if(unidades == 0 || unidades == ''){
        $('#cantidadProducto\\|' + id[1] + '\\|'+ id[2]).val(0);
    }      
}
...
function ultimosPedidos(articulo, cliente){
    console.log('llego');
    var parametros = {
    ...
    $.ajax({
        ...
        success: function(response) {            
            ...
            for(var i = 0; i < response.length; i++){
                html += '<tr class="articuloYaComprado">'+
                '<td>'+response[i].CodigoArticulo+'</td>' +
                '<td>'+response[i].DescripcionArticulo+'</td>' +
                '<td>'+response[i].FechaPedido+'</td>' +                
                '<td>'+parseFloat(response[i].UnidadesPedidas).toFixed(2)+'</td>' +
                '<td>'+parseFloat(response[i].Precio).toFixed(2)+'</td>' +
                '<td>'+parseFloat(response[i].Descuento).toFixed(2)+'</td>' +
                '<td>'+parseFloat(response[i].ImporteLiquido).toFixed(2)+'</td>' +                    
                '</tr>';
            }
            $("#ArticulosPedidosAnteriormente").empty()
            $("#ArticulosPedidosAnteriormente").append(html)
            $("#botonUltimosArticulos").trigger('click');
...
function actualizarDatos(codigoArticulo) {
    ...
    var orden = $("." + codigoABuscar[1] + "linea_articulo").val();
    console.log(orden);
    var nuevoValor = valorActual * nuevaCantidad;
    subTotal = nuevoValor;

    $("#" + codigoABuscar[1] + "1").val(subTotal);
    $("." + codigoABuscar[1] + "").children("td")[7].innerText = parseFloat(subTotal).toFixed(2);
...
function actualizarDatos2(codigoArticulo) {
    ...
    var codigoABuscar = codigoArticulo.split("|");
    var nuevaCantidad = $("." + codigoABuscar[1]+"|"+orden  + "cantidadProducto").val();
    descuento = $("." + codigoABuscar[1]+"|"+orden  + "dto_articulo").val();
    var valorActual = $("." + codigoABuscar[1]+"|"+orden  + "pvp_articulo").val();
    var orden = codigoABuscar[2];
    console.log(orden);
    var nuevoValor = valorActual * nuevaCantidad;
    subTotal = nuevoValor;


    $("#" + codigoABuscar[1]+"|"+orden  + "1").val(subTotal);
    $("." + codigoABuscar[1]+"|"+orden  + "").children("td")[7].innerText = parseFloat(subTotal).toFixed(2);
    $("#" + codigoABuscar[1]+"|"+orden  + "cantidad").val(nuevaCantidad);
    /**
     *
     */
    var nuevoDescuento = $("." + codigoABuscar[1] + "dto_articulo").val();
 ...
 * @param codigoArticulo
 */
 async function actualizarPvpProducto(codigoArticulo) {
    
    var codigoABuscar = codigoArticulo.split("|");
    var orden = codigoABuscar[2];

    var codigo = codigoABuscar[1];

    codigoABuscar[1] = codigoABuscar[1].replace("/", "\\/");
    codigoABuscar[1] = codigoABuscar[1].replace(/[.]/g, "\\.");

    var cantidad = parseFloat($("." + codigoABuscar[1]+"\\|"+orden + "cantidadProducto").val(),2);
    var nuevoDescuento = parseFloat($("." + codigoABuscar[1]+"\\|"+orden + "dto_articulo").val(),2);
    var valorActual = parseFloat($("." + codigoABuscar[1]+"\\|"+orden + "pvp_articulo").val(),2);
    var cliente = ($('#codigoCliente').val());
    var ivaArticulo = 0.21;
    var recargoProducto = 0;
    ...
    $.ajax({
        ...
        success: function(response) {
            console.log(response);
            if(response[0].Recargo != ''){
            recargoProducto = response[0].Recargo;
            }else{
                recargoProducto = 0;
            }
            ivaArticulo = response[0].GI;
        }
    })

    await sleep(1000);

    console.log('cliene'+cliente);
    console.log('cantidad'+cantidad);
    console.log('nuevoDescuento'+nuevoDescuento);
    console.log('valor'+valorActual);
    console.log('iva'+ivaArticulo);
    console.log('recargo'+recargoProducto);

    var valorDescuento = parseFloat(valorActual * (nuevoDescuento / 100),2);

    var descuentoCliente = parseFloat(valorActual - valorDescuento,2);

    //var descuentoCliente = parseFloat(valorConDescuentoIncluido - (valorConDescuentoIncluido * (descuento / 100)),2);

	var prontoPago = descuentoCliente * ($('#descuentoProntoPago').val()/100);

    descuentoCliente = descuentoCliente - parseFloat(prontoPago);

    var valorRecargo = parseFloat((descuentoCliente * cantidad) *(recargoProducto / 100),2);

    var valorConIva = parseFloat(descuentoCliente); 

    var valorIvaCantida = parseFloat(valorConIva * cantidad,2);

    $("." + codigoABuscar[1]+"\\|"+orden + "").children("td")[5].innerText = valorConIva.toFixed(2);
    $("." + codigoABuscar[1]+"\\|"+orden + "").children("td")[7].innerText = valorIvaCantida.toFixed(2);
    $("#" + codigoABuscar[1]+"\\|"+orden + "1").val(valorIvaCantida.toFixed(2));
    $("#" + codigoABuscar[1]+"\\|"+orden + "2").val(nuevoDescuento.toFixed(2));
    
    console.log('valoractual '+valorActual);
    console.log('nuevoDescuento '+nuevoDescuento);
    console.log('valorDescuento '+valorDescuento);
    //console.log('valorConDescuentoIncluido '+valorConDescuentoIncluido);
    console.log('valorConIva '+valorConIva);
    console.log('descuentoCliente '+descuentoCliente);


    $('#'+codigoABuscar[1]+'_'+orden+'recargo').val(valorRecargo.toFixed(2))
    $('#'+codigoABuscar[1]+'_'+orden+'prontopago').val(prontoPago.toFixed(2));
    //$('#cuadroRecargo').show();
    ...
    if (nuevoDescuento > -1) {
        var parametros = {
            ...
            "dtoArticulo": nuevoDescuento,
            "iva": ivaArticulo,
            "recargo": recargoProducto,
			"protopago": $('#descuentoProntoPago').val()/100,
            "_token": $("meta[name='csrf-token']").attr("content")
        };
        //console.log(parametros)
        //actualización en bbdd de la cantidad de producto
        $.ajax({
            data: parametros,
            //url: './actualizarCantidad',
            url: '/actualizarDescuentoArticulo',
            type: 'post',
            timeout: 7000,
            async: true,
            success: function(response) {
                //////////////QUITAMOS ESTO PORQUE COMO TRABAJAN MUCHAS VECES QUITANDO PRECIO AL ARTÍCULO NO LE INTERESA QUE APAREZCA
                // if (response < 22) {
                //     if(response <= 0){
                //         Swal.fire({
                //             title: response+'%',
                //             text: 'El precio del articulo esta por debajo del costo',
                //             icon: 'warning',
                //             confirmButtonText: 'Cerrar'
                //         });
                ...
                //         Swal.fire({
                //             title: response+'%',
                //             text: 'El precio del articulo esta cerca del costo',
                //             icon: 'info',
                //             confirmButtonText: 'Cerrar'
                //         });
                //     }
            ...
            error : function(xhr, status) {
                Swal.fire({
                    title: 'Error!',
                    text: 'error actualizar cantidad, descuento o precio',
                    icon: 'error',
                    confirmButtonText: 'Cerrar'
                })
...
function eliminarProductoTabla(id) {
    var articuloOrden = id.split('|');
    id = id.replace("/", "\\/");
    id = id.replace(/[.]/g, "\\.");
    id = id.replace(/[|]/g, "\\|");

    var dineroARestarProducto = $("." + id + "").children("td")[6].innerText;
    //console.log(dineroARestarProducto)
    var parametros = {
        "codigoArticulo": articuloOrden[0],
        "orden": articuloOrden[1],
        "seriePedido": $("#seriePedido").val(),
...
function recalcularLineasPedido(lineas){
    var baseImponible = '';
    var recargo = '';

    for(let i = 0; i<lineas.length; i++){
        id = lineas[i].CodigoArticulo;
        baseImponible = parseFloat(lineas[i].BaseImponible);
        recargo = parseFloat(lineas[i].CuotaRecargo);
        id = id.replace("/", "\\/");
        id = id.replace(/[.]/g, "\\.");
        
        
        //$("." + lineas[i][CodigoArticulo]+"-"+lineas[i][Orden] + "").children("td")[5].innerText = valorConIva;
        console.log($("." + id+"\\|"+lineas[i].Orden + "").children("td")[7].innerText);
        $("." + id+"\\|"+lineas[i].Orden + "").children("td")[7].innerText = parseFloat(baseImponible).toFixed(2)+'';
        $("#" + id+"\\|"+lineas[i].Orden + "1").val(parseFloat(baseImponible).toFixed(2));
        //$("#" + lineas[i][CodigoArticulo]+"-"+lineas[i][Orden] + "2").val(lineas[i][BaseImponible]);
        $("#" + id+"\\|"+lineas[i].Orden+"recargo").val(parseFloat(recargo).toFixed(2));
...
//comprobarLineas();
    ...
    $(".linea-terminada").find(".recargo_articulo").each(function() {
        console.log($(this).val());
        total_recargo += parseFloat($(this).val());
    });
    ...
    var pp=$('#descuentoProntoPago').val();    
    //var descuentoProntoPago = total * (parseFloat(pp)/100);
    var iva = total * 0.21; 
    var ptotal = (total) + parseFloat(iva)  + parseFloat(total_recargo) ;

    $('#totalRecargo').text(total_recargo.toFixed(2)+'');
    $("#totaliva").text(iva.toFixed(2)+'');
    $("#baseImponible").text(total.toFixed(2)+'');
    $("#precioTotal").text(ptotal.toFixed(2)+'€');        
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response[0] = "OK") {
                ...
                $("#codigoPedido").val(response[1]);
                if ($("#comentario").val() != '') {
                    guardarComentario($("#comentario").val())
                }
                if ($("#suPedido").val() != '') {
                    guardarSuPedido($("#suPedido").val())
                }
            } else {
                Swal.fire({
                            title: 'Error!',
        ...
        error : function(xhr, status) {
            Swal.fire({
                title: 'Error!',
                text: 'algo salió mal al insertar la linea',
                icon: 'error',
                confirmButtonText: 'Cerrar'
            });
            // var codigoArticulo = datosPedido
            // $('#linea-terminada ')
        },
...
function obtenerDatosPedido() {
    ...
    $(".linea-terminada").each(function() {
        ...
        linea.orden = $(this).find($(".ordenLinea")).val();
...
function comprobarUnidadesDevolucion(ejercicioAlbaran, serieAlbaran, numeroAlbaran) {
    ...
    $.ajax({
        data: parametros,
        url: './comprobarUnidadesDevolucion',
        type: 'post',
        timeout: 3000,
        async: true,
        success: function(response) {
...
function comprobarUnidadesQuedanPorDevolver(ejercicioAlbaran, serieAlbaran, numeroAlbaran) {
    ...
    $.ajax({
        data: parametros,
        url: './comprobarUnidadesQuedanPorDevolver',
        type: 'post',
        timeout: 3000,
        async: true,
        success: function(response) {
...
function seleccionarAlbaran(albaran) {
    ...
    $.ajax({
        data: parametros,
        url: './obtenerLineasAlbaranSeleccionado',
        type: 'post',
        timeout: 2000,
        async: true,
        success: function(response) {
...
function descripcion0 (valor, id){
...
function precioCosteProducto(valor, id){
    let e = id.split('|');
...
function comprobarLineas(){
    ...
    // console.log($('#lineasPedido').val());
    // console.log($('#descuentoProntoPago').val());
    // console.log($('#botondescuentoProntoPago'));
    // console.log($(".subTotal").length);
    if($('#lineasPedido').val() == 0 && $('#descuentoProntoPago').val() == 0){

    }else if($(".subTotal").length == $('#lineasPedido').val() && $('#descuentoProntoPago').val() > 0 || $(".subTotal").length == $('#lineasPedido').val() && $('#descuentoProntoPago').val() == 0 ) {

        $('#botondescuentoProntoPago').css('color', 'green')

    }else{

        $('#botondescuentoProntoPago').css('color', 'red')
