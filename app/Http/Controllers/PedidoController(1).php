...
class PedidoController extends Controller
{
    ...
    static function insercionPedido()
    {
        ...
        $existeCabeceraPedido = self::comprobarExisteCabeceraPedido($_POST['datos']['idPedido'], $_POST['datos']['seriePedido']);
    ...
    public static function comprobarExisteCabeceraPedido($numero, $serie)
    {
        $existe = false;
        $query = DB::table('CabeceraPedidoCliente')
            ->where('CodigoEmpresa', '=', session('codigoEmpresa'))
    ...
    public static function repetirPedido()
    {
        ...
        if ($cabeceraPedido != false) {
            ...
            foreach ($_POST['lineasPosicion'] as $lineas) {
                ...
                if ($lineas['origen'] == 1) {
                    ...
                    $lineo = DB::table('Articulos')
                        ...
                        ->where('CodigoArticulo', '=', $lineas['guid'])
                        ->where('CodigoEmpresa', '=', session('codigoEmpresa'))
                        ->get();

                        if($lineo[0]->TratamientoPartidas == -1){
                            $partidas = DB::table('AcumuladoStock')
                            ->select('Partida','FechaCaducidad','PecioCompra', '%Margen as Margen')
                            ->where('Periodo',99)
                            ->where('CodigoAlmacen','0')
                            ->where('CodigoEmpresa',\session('codigoEmpresa'))
                            ->where('Ejercicio',date('Y'))
            ...
            $contador = self::contadorMenos('', $cont, 'pedidos_cli');
            $ejercicio = date('Y');
