                        ...
                        <div class="flex mb-4">
                            ...
                            <div class="w-1/2 p-2 text-right">
                                <div class="company-details">
                                    ...
                                    <div>P.I. Cerro Cabeza Hermosa Avda. Príncipe de Asturias, 8P-8Q </div>
                                    <div>41500- Alcalá de Guadaira (Sevilla)</div>
                                    <div>954 251 425</div>
                                </div>
                                ...
                                <div class="flex mb-4">
                                    ...
                                    <div class="w-1/4 sm:3/8 p-1">
                                        <div class="p-3 md:p-5 bg-blue-300">                                        
                                            ...
                                            <div x-data="{ show: false }">
                                                <div class="flex justify-between mt-2">
                                                    <button @click={show=true} class="block w-full py-4 bg-blue-500 hover:bg-blue-700 text-center text-white font-bold font-heading uppercase rounded-md transition duration-200 mr-1">
                                                        FINALIZAR PEDIDO
                                                    </button>
...
<script type="text/javascript">
    ...
    $('#printInvoice').click(async function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                //console.log(response);
                var html ='<table class="w-full text-sm text-left text-gray-500 dark:text-gray-400" id="tabla-articulosPrint" border="0" cellspacing="0" cellpadding="0">'+
                                            '<thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">'+
                                                '<tr>'+
                                                   '<th scope="col" class="px-6 py-3" id="columnaCabeceraCodigo">'+
                                                ...
                                                '</tr>'+
                                            '</thead>'+
                                            '<tbody class="listadoArticuloPedidoPrint">';
    ...
    function showObservacionArticulo(div){   
        ...
        //console.log(div);
        div = div.split("|");
        var observacion = div[0].replace("/", "\\/");        
        observacion = div[0].replace(/[.]/g, "\\.");        
        console.log(observacion);             

        $('#'+observacion+'\\|'+div[1]).css('display', 'block');
    }
    ...
    function comprobacionPedido(){
        var datosPedido = {};
        datosPedido.lineas = [];        
        datosPedido.serie = $('#seriePedido').val() ;
        datosPedido.numero = $('#codigoPedido').val();
        datosPedido.cliente = $("#codigoCliente").val();
        datosPedido.prontroPago = $('#descuentoProntoPago').val();
        ...
        $(".linea-terminada").each(function() {
            ...
            linea.recargo = $(this).find($(".recargo_articulo")).val(); 
            linea.iva = $(this).find($(".iva_articulo")).val();
            linea.precioCompra = $(this).find($(".precioCompra_articulo")).val();
            if($(this).find($(".precioCoste")).val() != undefined ){          
                linea.precioCosteVarios = $(this).find($(".precioCoste")).val();
            }else{
                linea.precioCosteVarios = 0;
            }
            linea.observacionArticulo = $(this).find($(".observacionArticulo")).val();
            linea.partida = "";
...
<script src="{{asset('js/pedido3.js')}}"></script>