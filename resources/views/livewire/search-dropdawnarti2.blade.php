<div class="relative mt-3 md:mt-0" x-data="{ isOpen: true }" @click.away="isOpen = false">
    ...
    @if (strlen($search) >= 2)
        <div            
            class="z-50 absolute overflow-auto ml-1 mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm text-gray-900 articuloResultado-box "
            ...
            @if ($searchResults->count() > 0)
                <ul>
                    @foreach ($searchResults as $result)
                        <li class="relative border-b border-gray-700 p-1" id='articulo{{$result->DescripcionArticulo}}'>
                            <a href="#" id="{{$result->CodigoArticulo}}" name="{{ $result->DescripcionArticulo }}" onclick="selectCodigoArticulo2(this.id,this.name, '<?=$result->PrecioVenta?>', '<?=$result->IdArticulo?>')">                                
                                    <h2 class="text-md font-semibold text-gray-900">{{$result->CodigoArticulo}}                                              
                                        @if($result->StockReservado > 0)
                                        <span class=" ml-2 p-1 right rounded-full bg-green-500">{{round($result->StockReservado, 0)}}</span>
                                        @else
