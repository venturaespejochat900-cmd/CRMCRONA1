<div class="relative mt-3 md:mt-0" x-data="{ isOpen: true }" @click.away="isOpen = false">
    ...
    @if (strlen($search) >= 2)
        <div            
            class="z-50 absolute  ml-1 mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm text-gray-900 productoResultado-box"
            ...
            @if ($searchResults->count() > 0)
                <ul>
                    @foreach ($searchResults as $result)
                        <li class="border-b border-gray-700" id='tarifa{{$result->Familia}}'>
                            <a href="#" id="{{$result->CodigoArticulo}}" onclick="selectCodigoProducto(this.id,<?=$result->PrecioVenta?>)">
