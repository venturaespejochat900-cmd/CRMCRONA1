...
<div class="flex space-x-1 justify-around">    
    ...
    <?php
        ...
        $cfacturas = ClienteController::cabeceraFactura($MovPosicion);
                
        if($cfacturas != 'error'){
            $facturas = ClienteController::lineasFactura($cfacturas[0]->EjercicioFactura, $cfacturas[0]->SerieFactura, $cfacturas[0]->NumeroFactura);
        }
    ?>
@if($cfacturas != 'error')
    <div x-data="{ show: false }">
        ...
        <button @click={show=true} type="button" class="p-1 text-blue-600 hover:bg-blue-600 hover:text-white rounded has-tooltip">
            <span class='tooltip rounded shadow-lg p-1 bg-black text-white-500 -mt-8'>Factura</span>
            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path></svg>
        </button>
        </div>
        <div x-show="show" tabindex="0" class="z-40 overflow-auto left-0 top-0 bottom-0 right-0 w-full h-full fixed">
            <div  @click.away="show = false" class="z-50 relative p-3 mx-auto my-0 max-w-full " style="width: 1200px;">
                <div class="bg-white rounded shadow-lg border flex flex-col overflow-hidden">
                    ...
                    <div class="px-6 py-3 text-xl border-b font-bold">Factura</div>
                        <div class="p-6 flex-grow">
                            ...
                            <table class="w-full">
                                ...
                                <tbody class="bg-white">
                                    @foreach($cfacturas as $cabecera)
                                        <tr class="text-gray-700 ">
                                            <td class="px-4 py-3 border">
                                                <div class="flex items-center text-sm">
                                                    {{$cabecera->EjercicioFactura}}
                                                </div>
                                            </td>
                                            <td class="px-4 py-3 border">
                                                <div class="flex items-center text-sm">
                                                    {{$cabecera->SerieFactura}}
                                                </div>
                                            </td>
                                            <td class="px-4 py-3 border">
                                                <div class="flex items-center text-sm">
                                            ...
                                            <td class="px-4 py-3 border">
                                                <div class="text-right text-sm">
                                                    ...
                                                    {{number_format(round($cabecera->ImporteLiquido,2), 2, ',', '.')}}€
                                                </div>
                                            </td>
                                        </tr>    
                                    @endforeach
                            ...
                            <table class="w-full">
                                <thead>
                                <tr class="text-md font-semibold tracking-wide text-left text-gray-900 bg-gray-100 uppercase border-b border-gray-600">
                                    <th class="px-4 py-3">Codigo Articulo</th>
